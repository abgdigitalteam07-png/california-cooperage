...
- **[2026-02] Cloudflare Pages `_headers` file** — build-static.js now emits `/app/frontend/_headers` (15 blocks) applying: 1-year immutable cache on `/cc-assets/*` (images, WebP, .glb 3D models); `max-age=0, must-revalidate` on all HTML routes so content edits go live on next request after redeploy; 1-hour cache + explicit Content-Type on `/sitemap.xml` and `/robots.txt`. Per-slug (with & without trailing slash) blocks are ROUTES-derived. Verified by testing agent (48/48 checks pass).
